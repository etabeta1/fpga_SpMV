...
module AESL_automem_rowPointers (
    clk,
    rst,
    ce0,
    we0,
...
//------------------------Parameter----------------------
localparam
    TV_IN  = "../tv/cdatafile/c.SpMV.autotvin_rowPointers.dat", 
    TV_OUT = "../tv/rtldatafile/rtl.SpMV.autotvout_rowPointers.dat"; 
//------------------------Local signal-------------------
parameter DATA_WIDTH = 32'd 9;
parameter ADDR_WIDTH = 32'd 4;
parameter DEPTH = 32'd 11;
parameter DLY = 0.1;
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[1] == 0) && (ce0 == 1) && (ce1 == 1) && (we1[1] == 1) && (address0 == address1))
	      dout0[1*8+0:1*8] <= #DLY din1[1*8+0:1*8];
	  else if (ce0 == 1)
	      dout0[1*8+0:1*8] <= #DLY mem[address0][1*8+0:1*8];
        else ;
    end
end
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[1] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[1] == 0) && (address0 == address1))
            dout1[1*8+0:1*8] <= #DLY din0[1*8+0:1*8];
	  else if (ce1 == 1)
            dout1[1*8+0:1*8] <= #DLY mem[address1][1*8+0:1*8];
        else ;
    end
end
...
always @ (posedge clk) begin
    if((we0[1] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[1] == 1) && (address0 == address1))
        mem[address0][1*8+0:1*8] <= #DLY din1[1*8+0:1*8];
    else if ((we0[1] == 1) && (ce0 == 1))
        mem[address0][1*8+0:1*8] <= #DLY din0[1*8+0:1*8];
end

always @ (posedge clk) begin
    if ((ce1 == 1) && (we1[1] == 1))
        mem[address1][1*8+0:1*8] <= #DLY din1[1*8+0:1*8];
end
