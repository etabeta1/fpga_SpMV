...
//------------------------Parameter----------------------
`define TV_IN_gmem3 "../tv/cdatafile/c.SpMV.autotvin_gmem3.dat"
`define TV_OUT_gmem3 "../tv/rtldatafile/rtl.SpMV.autotvout_gmem3.dat"
...
parameter gmem3_C_DATA_BITWIDTH = 32'd 32;
parameter gmem3_transaction_depth = 32'd 10;
parameter gmem3_mem_depth = 32'd 10;
...
reg [gmem3_DATA_BITWIDTH - 1 : 0] gmem3_mem_3 [0: gmem3_mem_depth - 1] = '{default : 'h0}; 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("D:/Desktop/SpMV/hls_component/hls_component/hls/sim/verilog/AESL_axi_master_gmem3.v: Write request address %d exceed AXI master gmem3 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        3 : WDATA_tmp[j] = gmem3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            3 : gmem3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        3 : WDATA_tmp[j] = gmem3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        default: $display("The page_num of AXI write is not valid!");
                                    endcase
                                end
                            end
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            3 : gmem3_mem_3[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("D:/Desktop/SpMV/hls_component/hls_component/hls/sim/verilog/AESL_axi_master_gmem3.v: Read request address %d exceed AXI master gmem3 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            3 : RDATA_tmp <= gmem3_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            3 : RDATA_tmp <= gmem3_mem_3[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            default: $display("The page_num of AXI read is not valid!");
                        endcase
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem3_transaction_depth; i = i + 1) begin 
          ...
          if (i%1 == 0) begin
              case(mem_page)
                  0 : gmem3_mem_0[i/1] = mem_tmp;
                  1 : gmem3_mem_1[i/1] = mem_tmp;
                  2 : gmem3_mem_2[i/1] = mem_tmp;
                  3 : gmem3_mem_3[i/1] = mem_tmp;
